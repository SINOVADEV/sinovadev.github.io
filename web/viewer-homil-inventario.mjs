...
const defaultOptions = {
  ...
  cMapUrl: {
    value: "https://sinovadev.github.io/web/cmaps/",
    kind: OptionKind.API
  },
  disableAutoFetch: {
    value: false,
  ...
  standardFontDataUrl: {
    value: "https://sinovadev.github.io/web/standard_fonts/",
    kind: OptionKind.API
  },
  useSystemFonts: {
    value: undefined,
  ...
  workerSrc: {
    value: "https://sinovadev.github.io/build/pdf.worker.mjs",
    kind: OptionKind.WORKER
  }
};
{
  defaultOptions.defaultUrl = {
    ...
    value: G_VIEWER.fileData,
    kind: OptionKind.VIEWER
  };
  defaultOptions.sandboxBundleSrc = {
    value: "https://sinovadev.github.io/build/pdf.sandbox.mjs",
    kind: OptionKind.VIEWER
  };
...
class L10n {
  ...
  static #fixupLangCode(langCode) {
    ...
    langCode = "es-es"; // Obligamos a que el idioma sea español
...
class AnnotationEditorParams {
  ...
  #bindListeners({
    ...
    editorStampAddImage.addEventListener("click", () => {
      console.log("editorStampAddImage", this, this.pdfDocument?.annotationStorage);
      console.log("editorStampAddImage", this.pdfDocument?.annotationStorage);
...
class DrawLayerBuilder {
  ...
  cancel() {
    ...
    console.log("DrawLayerBuilder_cancel")
    this._cancelled = true;
    if (!this.#drawLayer) {
      return;
    }
...
class TextLayerBuilder {
  ...
  static #enableGlobalSelectionListener() {
    ...
    window.addEventListener("blur", () => {
      isPointerDown = false;
      const fileObjectBlur_ = G_VIEWER;//JSON.parse(<?= JSON.stringify(params) ?>);
      // Se valdia si existe la instancia para restablecer el modo si es alguno de edición
      if(G_PDF_VIEWER_INSTANCE && fileObjectBlur_ && fileObjectBlur_.view == "signature"){
        G_PDF_VIEWER_INSTANCE.pdfViewer._restoreOptionBar();
      }
      this.#textLayers.forEach(reset);
    }, {
      signal
...
class PDFPageView {
  ...
  update({
    ...
    const fileObjectUpdate_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);
    // Se valdia si existe la instancia para restablecer el modo si es alguno de edición
    if(G_PDF_VIEWER_INSTANCE && fileObjectUpdate_ && fileObjectUpdate_.view == "signature"){
      G_PDF_VIEWER_INSTANCE.pdfViewer._restoreOptionBar();
    }
  ...
  async draw() {
    ...
    if (!this.annotationLayer && this.#annotationMode !== AnnotationMode.DISABLE) {
      ...
      // Deshabilitamos la edición _ RUBENCHO
...
class PDFViewer {
  ...
  #initializePermissions(permissions) {
    ...
    console.log("permissions.includes(PermissionFlag.MODIFY_CONTENTS)", AnnotationEditorType)
    if (!permissions) {
      return params;
    }
  ...
  setDocument(pdfDocument) {
    ...
    console.log("permissionsPromise", permissionsPromise)
      const fileObject_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);
      
    console.log("Editor online", fileObject_)
    if(fileObject_.view == "checkElements") {
      $("#editorStamp").attr({class: "hidden"});
      $("#editorInk").attr({"title": "Dibujar"});
      $("#editorInk span").text("Dibujar");
      $("#download").text("Guardar");
      
      $("#download").attr({"title": "Dibujar"});
      $("#lblTitle").text("Check de elementos"); 
    }    
  ...
  /**
   * Permite restablecer la opcion del menú
  */
  _restoreOptionBar(){

    // Se realiza si lo que se esta realizando es una firma
    if(AnnotationEditorType.STAMP != this.#annotationEditorMode){

      // Ejecutamos los metodos que permiten restablecer la págna
      this._updateScrollMode();
      this.viewer.removeAttribute("lang");
      this.#hiddenCopyElement?.remove();
      this.#hiddenCopyElement = null;
      ...
      var mode_ = AnnotationEditorType.NONE;
      this.#annotationEditorMode = mode_;
      this.#annotationEditorUIManager.updateMode(mode_, null, false);
  ...
  #resizeObserverCallback(entries) {
   ...
   /**
   * Permite mostrar una ventana de alerta
  */
  showAlert(title_, text_, icon_, callback_){
  ...
  validateOption(data){
    ...
    // recorremos los elemento agregados al PDF
    for(let key_ in data){
      recordObject_ = data[key_];

      // Se valida si el nombre del elementos agregados son "stampEditor", "inkEditor"
      if(["stampEditor", "inkEditor"].indexOf(recordObject_["name"]) != -1){
        return false;
      }
    }

    // Por defecto debe marcar si
    return true;
  }
  ...
  set annotationEditorParams({
    ...
    value
     ...
     // Se valida si existe alguna algun elemento de timpo imagen
    if(fileObject_.view == "signature" && this.pdfDocument && this.pdfDocument.annotationStorage && this.pdfDocument.annotationStorage.size > 0){
 
 
      console.log("#storage", this.pdfDocument.annotationStorage["#storage"])
      var count = 0;
      var elements = this.pdfDocument.annotationStorage.editorStats
      // Obtenemos los datos para guardar la firma
    
      console.log("storage", elements)
      // Se valida si se debe o no activar la opción
      if(!this.validateOption(this.pdfDocument.annotationStorage.getAll())){
        this.showAlert("¡Advertencia!", "Solo puede agregar una imagen o una firma.", "error");
        // Se deja deshabilitado
        return false
      }
...
class Toolbar {
    ...
    const buttons = [{
      ...
      eventDetails: { 
        get mode() {
          ...
          // Se valida si se debe o no activar la opción
          if(_this.#validateOption_()){
            // Se habilita o deshabilita normalmente
            return classList.contains("toggled") ? AnnotationEditorType.NONE : AnnotationEditorType.INK;
          } else {
            _this.#showAlert("¡Advertencia!", "Solo puede agregar una imagen o firma.", "error");
            // Se deja deshabilitado
  ...
  */
  ...
  #validateOption_(){
    ...
    const fileObjectOption_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);
    // Se valdia si existe la instancia para restablecer el modo si es alguno de edición
    if(fileObjectOption_ && fileObjectOption_.view == "signature"){

      let recordObject_;
      console.log("G_PDF_VIEWER_INSTANCE", G_PDF_VIEWER_INSTANCE)
      // recorremos los elemento agregados al PDF
      for(let key_ in G_PDF_VIEWER_INSTANCE.allStorage){
        recordObject_ = G_PDF_VIEWER_INSTANCE.allStorage[key_];
...
const PDFViewerApplication = {
  ...
  /**
   * permite obter la cantidad de registros
  */
  get allStorage(){
    return this.pdfDocument?.annotationStorage.getAll() || {};
  },
  appConfig: null,
  pdfDocument: null,
  pdfLoadingTask: null,
  ...
  async initialize(appConfig) {
    // Deshabilita la opción de copiar - RUBEN*
    document.addEventListener('keydown', function (e) {
      // Detectar Ctrl o Cmd (metaKey en Mac) junto con C (keyCode 67) o V (keyCode 86)
      if ((e.ctrlKey || e.metaKey) && (e.keyCode === 67 || e.keyCode === 86)) {
        e.preventDefault(); // Evitar la acción predeterminada (copiar/pegar)
      }
    });

    this.appConfig = appConfig;
    G_PDF_VIEWER_INSTANCE = this;
    try {
    ...
    mode = "is-dark"; // Obligamos a que el estilo sea oscuro para resaltar la página PDF
    if (mode) {
  ...
  /**
   * Permite almacenar la firma de correo
  */
  async saveInServer_(options = {}, fileObject_ = {}) {
    const _this = this;
    
    if (_this._saveInProgress) {
      return;
    }
    _this._saveInProgress = true;
    await _this.pdfScriptingManager.dispatchWillSave();
    try {
      _this.managerLoader_(true);
      const data = await _this.pdfDocument.saveDocument();
     
      // Obtenemos el base64 dell archivo actual
      fileObject_.newBase64 = _this.uint8ArrayToBase64_(data);

       

      // Permite actualizar el archivo PDF
      google.script.run.withSuccessHandler(function(response_){
        
        // Ocultamos el loader
        _this.managerLoader_(false);
        ...
        if(response_.success){ 
          ...
          // MOstramos alerta
          _this.showAlert("Mensaje de confirmación", response_.message, "success", function(){
            

            // Agregamos la cabecera
            $('head').append(`<link href="https://cdnjs.cloudflare.com/ajax/libs/mdb-ui-kit/7.3.2/mdb.min.css" rel="stylesheet"/>`);

             // Datos de la peticion
            const fileObject_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);

            // Validamos si es para check de elemtnos 
            if(fileObject_.view == "checkElements") {
                ...
                $("body").html(`
                  ...
                  <div class="container">
                    <div class="card">
                      <div class="card-header text-bg-success">Información</div>
                      <div class="card-body">
                        <h5 class="card-title">Documento de check</h5>
                        <p class="card-text">El documento guardado con éxito y almacenado correctamente. Para continuar con el siguiente paso del proceso, por favor ingrese a la aplicación. Si necesita asistencia adicional, no dude en ponerse en contacto con nuestro equipo de soporte.</p>
                      </div>
                    </div>
                  </div>
                `);
              }else if(fileObject_.view == "signature") { 
                ...
                $("body").html(`
                  ...
                  <div class="container">
                    <div class="card">
                      <div class="card-header text-bg-success">Información</div>
                      <div class="card-body">
                        <h5 class="card-title">Documento ya firmado</h5>
                        <p class="card-text">El documento ha sido firmado con éxito y almacenado correctamente. Para continuar con el siguiente paso del proceso, por favor ingrese a la aplicación. Si necesita asistencia adicional, no dude en ponerse en contacto con nuestro equipo de soporte.</p>
                      </div>
                    </div>
                  </div>
                `);
          ...
          // MOstramos alerta
          _this.showAlert("Mensaje informativo", response_.message, "warning");
        }
      }).updateFileInDrive(fileObject_);
      
    } catch (reason) {
      console.error(`Error when saving the document: ${reason.message}`);
    } finally {
      await _this.pdfScriptingManager.dispatchDidSave();
      _this._saveInProgress = false;
    }
    if (_this._hasAnnotationEditors) {
      _this.externalServices.reportTelemetry({
        type: "editing",
        data: {
          type: "save",
          stats: _this.pdfDocument?.annotationStorage.editorStats
        }
      });
    }
  },
  ...
  */
  ...
  /**
   * Permite convertir el arrayunit8
  */
  uint8ArrayToBase64_(uint8Array_) {
    let binaryString = "";
    for (let i = 0; i < uint8Array_.length; i++) {
        binaryString += String.fromCharCode(uint8Array_[i]);
    }
  ...
  /**
   * Permite mostrra u ocultar el loader
  */
  managerLoader_(isShow_){
    // Ocultamos los paneles anteriores
    Swal.close();

    // Se miestra el loader
    if(isShow_){
      
      // Datos de la peticion
      const fileObject_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);

      // Validamos si es para check de elementos 
      if(fileObject_.view == "checkElements") {

        // Permite mostrar el loader
        Swal.fire({
          title: "Guardando documento",
          html: "Se está almacenando la verificación de elementos en el documento actual. Este proceso puede tardar unos momentos. Por favor, espere mientras garantizamos que su documento sea registrado de manera segura.",
        ...
        Swal.fire({
          ...
          html: "Se está almacenando la firma en el documento actual. Este proceso puede tardar unos momentos. Por favor, espere mientras garantizamos que su firma sea registrada de manera segura.",
          timerProgressBar: true,
          didOpen: () => {
            Swal.showLoading();
          }
  ...
  */
  async downloadOrSave(options = {}) {
    ...
    // Obtenemos los datos para guardar la firma
    const fileObject_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);
    console.log("fileObject_", fileObject_)
    console.log("this.pdfDocument?.annotationStorage", this.pdfDocument?.annotationStorage)

    // Obtenemos la cantidad de elemento dibujados que no sean texto
    const drawElements_ = await this.verifyAllElements(this.pdfDocument?.annotationStorage);

    // Se valida si existe alguna firma agregada
    if(drawElements_ > 0){
      classList.add("wait");
      //await (this.pdfDocument?.annotationStorage.size > 0 ? this.save() : this.download());
      //Se procede a guardar
      await this.saveInServer_(options, fileObject_);
    ...
    if (fileObject_.view == "checkElements") {
      // Mostramos alerta
      this.showAlert("¡Advertencia!", "Por favor realiza el check de elementos.", "error");

    }else if(fileObject_.view == "signature"){
      // Mostramos alerta
      this.showAlert("¡Advertencia!", "Por favor agrega la firma al documento.", "error");
    }
    }
  },

  /**
   * Permite validar cuantos elemento diferentes a "freeTextEditor", existen
  */
  async verifyAllElements(annotationStorage_){
    let counter_ = 0;

    // Se valida que existan datos de elementops dibujados
    if(annotationStorage_ && annotationStorage_.size > 0){
      const elements_ = annotationStorage_.getAll() || {};

      // Recorremos y definimos un contador
      for(let key_ in elements_){
        if(elements_[key_] && elements_[key_].name && elements_[key_].name != "freeTextEditor"){
          counter_ ++;
        }
      }
    }
...
{
  const HOSTED_VIEWER_ORIGINS = ["null", "http://mozilla.github.io", "https://mozilla.github.io", "https://sinovadev.github.io", "http://sinovadev.github.io"];
  ...
  if (cmd === 0) {
    ...
    switch (evt.keyCode) {
      ...
      case 27:
        ...
        const fileObjectEscape_ = G_VIEWER; //JSON.parse(<?= JSON.stringify(params) ?>);
        // Se valdia si existe la instancia para restablecer el modo si es alguno de edición
        if(G_PDF_VIEWER_INSTANCE && fileObjectEscape_ && fileObjectEscape_.view == "signature"){
          G_PDF_VIEWER_INSTANCE.pdfViewer._restoreOptionBar();
        }
        break;
      case 40:
...
const pdfjsVersion = "4.6.6";
const pdfjsBuild = "038e48276";
const AppConstants = {
